...
DEFAULT_OUTPUT = Path(
    "data/real_ball_friction_8env_60f_1prefix9impact_20260809"
)
ENVIRONMENTS = (
    ("ball-0_lerobot", 0),
    ("ball-1_lerobot", 1),
...
def build_rows(source_root: Path) -> tuple[list[dict], dict[str, dict]]:
    ...
    for environment_index, (environment, ball_id) in enumerate(ENVIRONMENTS):
        ...
        expected_gears = set(range(1, 11))
        if set(gear_episode_counts) != expected_gears:
            raise ValueError(
                f"{environment}: expected skill levels 1-10, got {sorted(gear_episode_counts)}."
            )
