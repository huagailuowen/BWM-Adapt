...
cd "$ROOT"
CONFIG=${1:?TTT training config required}
...
export BWM_TTT_DEADLINE_EPOCH=$(($(date -d "$START_TEXT" +%s) + 84600))
printf '[allocation] start=%s checkpoint_epoch=%s output=%s\n' "$START_TEXT" "$BWM_TTT_DEADLINE_EPOCH" "$RUN"
...
import yaml
config, output, cache = map(Path, sys.argv[1:])
flat = yaml.safe_load(config.read_text())["training"]
...
shutil.copy2(config, output / "submitted_config.yaml")
active = frozen / "ttt_active_environments.yaml"
active.write_text(yaml.safe_dump({"selection": {
    "active_environment_ids": sorted(summary["environment_ids"].values())},
...
flat.update(dataset_base_path=str(local), dataset_metadata_path=str(frozen / "train.jsonl"),
            ...
            ttt_active_environment_manifest=str(active))
...
(output / "allocation.json").write_text(json.dumps({
    ...
    "wallclock_checkpoint_epoch": int(os.environ["BWM_TTT_DEADLINE_EPOCH"]),
...
exec .venv/bin/python -m torch.distributed.run \
  ...
  --rdzv_id="${SLURM_JOB_ID}_ttt" "${BWM_TTT_TRAIN_ENTRYPOINT:-scripts/methods/train_real97_ttt.py}" --config "$RUN/runtime.yaml"
