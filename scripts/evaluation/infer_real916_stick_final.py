#!/usr/bin/env python3
"""Matched 33x3 Stick rollout, with training-faithful masked support loss."""
import argparse
import copy
import fcntl
import hashlib
...
def main():
    ...
    write_json(output/'provenance.json',{'config':config,'method':own.method,'model_step':setting['step'],
               ...
               'known_environment_initialization':own.method=='ours','context_hard_bounds':None,
    ...
    for env in plan['environments']:
        ...
        if own.method=='ours':
            if state_path.exists():
                ...
                noise=(torch.rand(target.shape,generator=generator,dtype=torch.float32)-.5).to(pipe.device)
                ...
                state={'context':adapted.detach().cpu().tolist(),'initial':initial.cpu().tolist(),'target':target.cpu().tolist(),
                       'noise_seed':seed,'noise_range':[-.5,.5],'trajectory':trajectory,'losses':losses,'metrics':metrics,
                       'support_indices':env['support_indices'],'initialization':'corresponding_environment_table_plus_uniform_noise'}
