#!/usr/bin/env python3
"""Score LoRA with the frozen Real97 metrics; never modify baseline results."""
...
def freeze_manifest(config, output):
    ...
    for task, setting in config["tasks"].items():
        run = engine.ROOT / setting["lora"]
        completion = engine.read_json(run / "inference_complete.json")
        if completion["checkpoint_step"] != setting["lora_checkpoint_step"]:
            raise RuntimeError(f"Unexpected LoRA checkpoint: {task}")
        indexed = {}
        for marker in sorted((run / "completed").glob("q*.json")):
            ...
            if identity in indexed:
                raise RuntimeError(f"Duplicate LoRA query: {identity}")
        ...
        if set(indexed) != {q["row"]["sample_id"] for q in reference}:
            raise RuntimeError(f"LoRA and baseline query sets differ for {task}; no silent intersection")
        for query in reference:
            ...
            paths = {"gt": query["paths"]["gt"], "lora": record["prediction"]}
            for video in paths.values():
                if not Path(video).is_file():
                    raise FileNotFoundError(video)
            queries.append({**query, "paths": paths, "source_markers": {
                "lora": str(marker), "baseline": query.get("source_markers", {})}})
        if completion["queries"] != len(reference):
            raise RuntimeError(f"Incomplete LoRA query publication: {task}")
        steps[task] = {"lora": completion["checkpoint_step"]}
    manifest = {
        "config": config, "queries": queries, "methods": ["lora"], "paired_gt_only": True,
        ...
        "fairness_note": (
            ...
            "LoRA starts from Standard step5500; TTT and historical Ours checkpoints differ. "
...
def aggregate(output, manifest):
    ...
    report["action"].append(ball_action(scored, "lora"))
    ...
    comparison = {
        "status": config["tracking_status"], "lora_queries_expected": len(manifest["queries"]),
        "lora_queries_scored": len(scored), "lora_lpips_complete": all(
        ...
        "checkpoint_steps": {"baseline": baseline_manifest["checkpoint_steps"],
                             "lora": manifest["checkpoint_steps"]},
    ...
    ttt_source = engine.ROOT / config["ttt_scores"]
    ttt_report = engine.read_json(ttt_source / "summary_provisional.json")
    ttt_manifest = engine.read_json(ttt_source / "manifest.json")
    expected = {(q["task"], q["row"]["sample_id"]) for q in manifest["queries"]}
    actual = {(q["task"], q["row"]["sample_id"]) for q in ttt_manifest["queries"]}
    if expected != actual:
        raise RuntimeError("TTT/LoRA score populations differ")
    comparison["image_object"] += ttt_report["image_object"]
    comparison["action"] += ttt_report["action"]
    comparison["checkpoint_steps"]["ttt"] = ttt_manifest["checkpoint_steps"]
    comparison["ttt_lpips_complete"] = all(
        (ttt_source / "lpips" / q["task"] / (q["key"] + ".json")).is_file()
        for q in ttt_manifest["queries"])
    engine.save_json(output / "comparison_with_baselines.json", comparison)
    print(json.dumps({"scored": len(scored), "ball_lora_action": report["action"][-1]["macro_score"],
                      "lpips_complete": comparison["lora_lpips_complete"]}), flush=True)
    return report
