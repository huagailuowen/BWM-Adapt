...
#SBATCH --requeue
#SBATCH --output=logs/real97-standard-reference-%j.out
#SBATCH --error=logs/real97-standard-reference-%j.err
set -euo pipefail
...
exec .venv/bin/python scripts/evaluation/infer_real97_standard_reference.py \
  --task "${1:?stick or soft required}" \
  --config configs/evaluation/real97_standard_reference_20260911.json \
  --output "outputs/infer_real97_${1}_standard_step5500_reference_job${SLURM_JOB_ID}"
