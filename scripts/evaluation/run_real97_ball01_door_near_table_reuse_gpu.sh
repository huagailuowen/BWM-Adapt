#!/usr/bin/env bash
#SBATCH --job-name=ball01-door-near-Z
...
#SBATCH --requeue
#SBATCH --output=logs/ball01-door-near-Z-%j.out
#SBATCH --error=logs/ball01-door-near-Z-%j.err
set -euo pipefail
ROOT=/hai/scratch/cyzhou05/projects/TTT-Physics/repos/BWM-Adapt
cd "$ROOT"
CONFIG=configs/evaluation/real97_ball01_door_near_table_unbounded_20260911_v2.json
...
run_task() (
    ...
    task="$1"
    output_id=114807
    if [[ "$task" == door ]]; then output_id=114799; fi
    ...
    [[ -d "$output" ]] || { printf 'Missing existing output: %s\n' "$output" >&2; return 2; }
    exec 8>"$output/.support_revision.lock"
    flock -n 8 || return 2
    python3 scripts/evaluation/prepare_real97_support_override.py prepare \
        --config "$CONFIG" --task "$task" --job-id "$output_id" --reuse-output-job-id || return "$?"
...
status=0
for task in ball door; do
    if run_task "$task"; then
        printf '[near-table] completed %s\n' "$task"
    else
        printf '[near-table] failed %s; the other task will still be attempted\n' "$task" >&2
        status=1
    fi
done
exit "$status"
