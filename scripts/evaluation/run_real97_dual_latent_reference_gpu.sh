...
#SBATCH --output=logs/real97-dual-mean-infer-%j.out
#SBATCH --error=logs/real97-dual-mean-infer-%j.err
...
CONFIG=configs/evaluation/real97_ball_door_dual_latent_mean_20260913_v1.json
...
exec "$ROOT/.venv/bin/python" scripts/evaluation/infer_real97_dual_latent_reference.py \
  --config "$CONFIG" --task "$TASK" \
  --output "outputs/infer_real97_${TASK}_dual_z0_meanall_step5500_job${SLURM_JOB_ID}"
