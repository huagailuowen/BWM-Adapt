#!/usr/bin/env python3
"""Score dual6000 against frozen factual Door queries and unchanged calibration."""
import json
...
def freeze_manifest(config, output):
    ...
    if (done['model_step'], done['table_step']) != (6000, 6000):
        raise RuntimeError('Expected paired 6000-step model and table')
    indexed = {}
    ...
    result = dict(config=config, queries=queries, methods=list(engine.METHODS), paired_gt_only=True,
                  checkpoint_steps={'door':{'ours_stage1':6000,'ours_stage2':6000}},
