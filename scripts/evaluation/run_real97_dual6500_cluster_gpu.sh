...
#SBATCH --output=logs/dual6500-cluster-%j.out
#SBATCH --error=logs/dual6500-cluster-%j.err
set -euo pipefail
...
CONFIG="configs/evaluation/real97_${TASK}_dual6500_cluster_mean_20260917_v1.json"
.venv/bin/python scripts/evaluation/prepare_real97_dual6500_cluster_reference.py --config "$CONFIG" --task "$TASK"
RESOLVED="outputs/evaluation_real97_${TASK}_dual6500_cluster_mean_20260917_v1/resolved_config.json"
exec .venv/bin/python scripts/evaluation/infer_real97_dual_latent_reference.py \
  --config "$RESOLVED" --task "$TASK" \
  --output "outputs/infer_real97_${TASK}_dual6500_cluster_mean_20260917_v1"
