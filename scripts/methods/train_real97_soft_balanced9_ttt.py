#!/usr/bin/env python3
"""Opt-in physical-nine Soft sampling and safe saving around the existing TTT causal scan.
...
class Real97Streams:
    def __init__(self, *, metadata_path, seed, sequence_length, streams_per_environment=1, **kwargs):
        ...
        self.pool = PooledEpisodeSampler(SimpleNamespace(
            ...
            standard_environments_per_rank=5,
    ...
    def sample(self, *, step, process_index, num_processes, environments_per_rank):
        if int(num_processes) != 2 or int(environments_per_rank) != 5:
            raise ValueError("Soft TTT requires two ranks and five environments per rank.")
        ...
        if self.stop_flag is None:
            ...
            if accelerator.is_main_process:
                ...
                atomic_json(self.output / "ttt_run_settings.json", {
                    ...
                    "global_chunks_per_optimizer_update": 60,
        ...
        if self.step % int(self.cfg["log_steps"]) == 0:
            ...
            if accelerator.is_main_process:
                record = {
                    ...
                    "timestamp": time.time(), "global_chunks": 60,
                }
...
def main():
    ...
    if batch_layout != (6, 5, 1):
        raise ValueError("Expected per-rank five environments and one six-chunk stream each.")
    ...
    legacy.build_dataset = build_static_dataset
