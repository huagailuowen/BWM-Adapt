#!/usr/bin/env bash
#SBATCH -A yejin
#SBATCH --job-name=mb30fp-near-unbal
#SBATCH -p yejin-lo
#SBATCH --gres=gpu:1
...
TRAIN_JOB=110422
TRAIN_OUT=outputs/mass_balance_fixed_pose_30ratio_train20_mainview_4env8chunk_c32_oldrandom_stage1_4300_${TRAIN_JOB}
...
OUT_DIR=results/mass_balance/fixed_pose_30ratio_noleak_nearest_unbalanced_center_support_dense15_v1/methods/ours/step_3900/seed_20260723
...
LOCAL_ROOT=/tmp/${USER}/mass_balance_fixed_pose_30ratio_noleak_eval_${SLURM_JOB_ID}
...
dataset_protocol=fixed_pose_30ratio_train20_noleak_mainview
checkpoint=$CKPT
context_table=$TABLE
config=$CONFIG
...
.venv/bin/python scripts/analyze_context_trajectory_active_pca.py \
  ...
  --title "No-leak fixed-pose mass balance C32: training-time Z table and inference-time trajectories"
...
ACTION_RESULT=${OUT_DIR}/action_evaluation
